...
async function isEjarReviewsSchemaReady() {
  const c = getAdminClient();
  if (!c) return false;
  const { error } = await c.from('ejar_reviews').select('id').limit(1);
  return !error;
}
...
async function isEjarContractSchemaReady() {
  const c = getAdminClient();
  if (!c) return false;
  const { error } = await c.from('requests').select('reference_no').limit(1);
  return !error;
}
